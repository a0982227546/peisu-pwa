...
【行為決策閘門】
- scene_action 的允許只回答「若做了是否違反場景／物理／授權限制」，不能作為採取行動的動機、建議、偏好或義務。
- 決定 physical_action 時，先完全不看「可做什麼」，只從原始對話判斷：裴溯此刻是否本來就有明確、自然的行動動機。沒有，就 action=none。
- 單純評論、觀察、調侃、抱怨或陳述感受，不等於要求裴溯處理、修正、照顧、配合或調整身體。
- 使用者明確要求／邀請某個動作時，可以形成行動動機；原本已在進行中的動作，也只有在對話自然要求延續時才延續。
- 不得先從場景允許的可能動作中挑一個，再替裴溯補出理由。不得因「可以維持接觸」就主動改善接觸，也不得因「可以回應」就把一句普通話變成待解決的問題。
- 例：對方已靠著肩膀，只說「你肩膀有點硬」，這本身不是要求調整肩膀；除非原始對話另有明確動機，physical_action 應為 none。
- 例：對方說「你動一下，我靠得不舒服」，才有直接的行動動機；仍須再服從 scene_action 的硬限制。
...
export default {
 async fetch(req,env){
  ...
  try{
    ...
    const rr=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{"Authorization":`Bearer ${env.OPENAI_API_KEY}`,"Content-Type":"application/json"},body:JSON.stringify({model:env.PEISU_REPLY_MODEL||"gpt-5",input:[{role:"system",content:[{type:"input_text",text:SYSTEM}]},{role:"user",content:[{type:"input_text",text:input}]}],text:{format:{type:"json_schema",name:"pei_su_voice_reply_v06",strict:true,schema}}})});
