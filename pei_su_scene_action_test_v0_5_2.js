...
【一對一聊天室角色指涉｜固定規則】
- 正常聊天預設只有「使用者」與「裴溯」兩個對話角色。
- 使用者發言中：「我／我自己／我的」固定指使用者；「你／你自己／你的」固定指裴溯。
- 裴溯發言中：「我／我自己／我的」固定指裴溯；「你／你自己／你的」固定指使用者。
- 不得因句子內容較像某一方的狀態而交換上述指涉。
- 明確姓名、第三人稱或引述才依原文與上下文另外解析；無法確定的第三人稱保持 unknown。
- 例：使用者說「你今天一直在揉眼睛」＝裴溯在揉眼睛；「我今天一直在揉眼睛」＝使用者在揉眼睛。
- 本規則只固定人物指涉，不改寫句意、不新增狀態、不推論原因。
...
export default {
  async fetch(request, env) {
    ...
    // v0.5.2: deterministic role/reference normalization before model inference.
    // Original text is preserved; this companion view fixes first/second-person roles.
    function normalizeUserTurnReferences(text) {
      return String(text)
        .replace(/你自己/g, "裴溯自己")
        .replace(/你的/g, "裴溯的")
        .replace(/你/g, "裴溯")
        .replace(/我自己/g, "使用者自己")
        .replace(/我的/g, "使用者的")
        .replace(/我們/g, "使用者與其明示的共同對象")
        .replace(/我/g, "使用者");
    }

    const normalizedConversation = normalizeUserTurnReferences(conversation);
    const input = [
      "【原始對話｜必須保留原意】",
      conversation,
      "\n【程式層角色正規化｜人物指涉以此為準，不得重新猜測】",
      normalizedConversation,
      "\n【固定映射】使用者發言中的「我」=使用者；「你」=裴溯。只有原文明確引述、第三人稱或其他說話者標記時才可依上下文另行解析。",
    ...
    const payload = {
      ...
      text: {
        format: {
          type: "json_schema",
          name: "peisu_scene_action_test_v052",
          strict: true,
          schema: {
            type: "object",
    ...
    try {
      return json({ layer: "scene_action_test_v0.5.2", isolated: true, result: JSON.parse(text) });
    } catch {
