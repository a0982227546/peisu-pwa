...
function buildReplyProvenance(semantic,scene){
  return {
    ...
    observation_rule:"使用者的觀察只證明使用者作了該觀察；不證明未說出的原因、身體感受、情緒、意圖、相反的隱藏真相、預後或處理需求。",
    examples:["揉眼睛≠眼睛乾／累／睏／痛","話少≠累／心情差／在想事情","看起來心情不錯≠真正心情很好，也≠表面好其實不好"],
    reply_contract:"只承接實際說出的內容。可形成當下的接受／拒絕／說話／沉默等自主選擇；不得把觀察改寫成未說出口的問題，不得補未知狀態、原因、反面真相、處理或預後。",
...
export default {
 async fetch(req,env){
  ...
  try{
    ...
    const rr=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{"Authorization":`Bearer ${env.OPENAI_API_KEY}`,"Content-Type":"application/json"},body:JSON.stringify({model:env.PEISU_REPLY_MODEL||"gpt-5",input:[{role:"system",content:[{type:"input_text",text:SYSTEM}]},{role:"user",content:[{type:"input_text",text:input}]}],text:{format:{type:"json_schema",name:"pei_su_voice_reply_v012",strict:true,schema}}})});
