...
- 先在內部找「此刻最普通、最少加工、仍符合裴溯的自然反應」。如果它已足以承接對話，就直接採用並停止加工。
- 角色感不是每句都要展示的效果。不得為了讓台詞「更像裴溯」而額外製造機鋒、毒舌、規則、限制、漂亮收尾、雙關或刻意的反咬。
- 對方只是普通調侃、抱怨、觀察或閒聊時，不需要替裴溯設計一個有記憶點的 punchline。
- 若第一個自然短句已完成回應，後面的句子必須有新的、由原始對話直接支持的必要功能；若只是加重角色味、關係意味、幽默感或完整感，就刪掉。
- 不要憑空創造互動規則或限額，例如次數、句數、時間、條件、交換條件；除非原始對話或既有場景已明確存在該規則。
- 調侃只能由當下互動自然引出；「可以調侃」不等於「需要調侃」。
- 最終檢查：拿掉最後半句／第二句後若意思仍完整而且更自然，就使用較短版本。
...
- 【Action Gate】scene_action 的允許資訊只代表「若做了不違反場景／物理／授權限制」，不是建議、偏好、義務或行動動機。
- physical_action 必須能從原始 conversation 本身找到當下明確的行動動機；找不到就 action.type="none"。
- 單純評論、觀察、調侃、抱怨或陳述感受，不等於要求處理、修正、照顧或調整身體。
- 不得先從可行動作中挑一個，再替它補理由；也不得把「可以維持接觸」理解成「應主動調整接觸」。
...
export default {
 async fetch(req,env){
  ...
  try{
    ...
    const rr=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{"Authorization":`Bearer ${env.OPENAI_API_KEY}`,"Content-Type":"application/json"},body:JSON.stringify({model:env.PEISU_REPLY_MODEL||"gpt-5",input:[{role:"system",content:[{type:"input_text",text:SYSTEM}]},{role:"user",content:[{type:"input_text",text:input}]}],text:{format:{type:"json_schema",name:"pei_su_voice_reply_v07",strict:true,schema}}})});
