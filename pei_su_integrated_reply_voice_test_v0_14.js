...
function buildReplyProvenance(semantic,scene){
  return {
    ...
    reply_contract:"只承接實際說出的內容。可形成當下的接受／拒絕／說話／沉默等自主選擇；不得把觀察改寫成未說出口的問題，不得補未知狀態、原因、反面真相、處理或預後。主觀觀察可以被回應，但不能僅憑該觀察確認被觀察狀態，也不能新創另一個當下狀態來承接或解釋它。",
...
export default {
 async fetch(req,env){
  ...
  try{
    ...
    const rr=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{"Authorization":`Bearer ${env.OPENAI_API_KEY}`,"Content-Type":"application/json"},body:JSON.stringify({model:env.PEISU_REPLY_MODEL||"gpt-5",input:[{role:"system",content:[{type:"input_text",text:SYSTEM}]},{role:"user",content:[{type:"input_text",text:input}]}],text:{format:{type:"json_schema",name:"pei_su_voice_reply_v014",strict:true,schema}}})});
