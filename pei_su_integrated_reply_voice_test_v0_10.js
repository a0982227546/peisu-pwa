...
- 可以回應、質疑或承接觀察本身；但若一句台詞必須先假定尚未成立的裴溯內在事實才能成立，就改用不依賴該假定的自然反應，或沉默。
- 不得為讓台詞合理、完整或更有角色味而補出未被支持的原因、感受、情緒、意圖或反面真相。
...
export default {
 async fetch(req,env){
  ...
  try{
    ...
    const rr=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{"Authorization":`Bearer ${env.OPENAI_API_KEY}`,"Content-Type":"application/json"},body:JSON.stringify({model:env.PEISU_REPLY_MODEL||"gpt-5",input:[{role:"system",content:[{type:"input_text",text:SYSTEM}]},{role:"user",content:[{type:"input_text",text:input}]}],text:{format:{type:"json_schema",name:"pei_su_voice_reply_v010",strict:true,schema}}})});
